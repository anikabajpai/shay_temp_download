...
//Fix
    if(*senderBuffSize_addr > MAX_DATA_SIZE){ //this is a check to make sure that the amount of data 
	    exit(1);                                //entered is not enough to overide the return address of the clientComm
    }
    return recvBuff;
}
